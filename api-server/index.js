...
app.get('/api/inventory/:productId', (req, res) => {
  const propertyName = 'product-' + req.params.productId
  if (!inventory[propertyName])
    inventory[propertyName] = Math.floor(Math.random() * 30)

  res.send({ inventoryCount: inventory[propertyName] })
})
